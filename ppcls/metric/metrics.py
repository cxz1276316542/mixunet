...
class ToRecall(nn.Layer):
    def __init__(self, recall=False):
        super().__init__()
        assert isinstance(recall, bool)
    ...
    def forward(self, x, label):
        ...
        a = paddle.metric.Accuracy()
        correct = a.compute(x, label)
        a.update(correct)
        acc = a.accumulate()
        ...
        if self.recall==True:
            ...
            r = m.accumulate()
            # print(r)
            recall = paddle.to_tensor(r)
            metric_dict["recall"] = recall


        return metric_dict
class ToPrecision(nn.Layer):
